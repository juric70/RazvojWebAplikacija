<template>
    <div class="exercisesMainDiv">
        ...
        <tr>
            ...
            <th>Cijena </th>
            <th>Kreator</th>
            <th v-if="this.store?.user?.role == 1 " colspan="2">Akcije</th>
        </tr>
        <tr  v-for="program in Programs" :key="program.id">
            ...
            <td v-if="this.store?.user?.role == 1 "><router-link :to="{name: 'ModifyProgram', params: {id : program.id}}">Uredi</router-link></td>
            <td v-if="this.store?.user?.role == 1 "><router-link :to="{name: 'DeletePrograms', params: {id : program.id}}">Obrisi</router-link></td> 
        </tr>
...
mounted(){
      axios.get('/api/programs').then((res) => {
        ...
        console.log("ovo je konzollog: ", res.data.output)
      })
    }
}
</script>
